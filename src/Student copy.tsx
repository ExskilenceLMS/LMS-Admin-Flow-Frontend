...
const Student: React.FC = () => {
  ...
  const fetchModalBatches = async () => {
    try {
      ...
      setModalBatches(response.data.batches);
      setLoading(false);
    } catch (error) {
  ...
  const handleSave = async (): Promise<void> => {
    if (!formData.batch_id || !formData.student_firstname || !formData.student_lastname || !formData.student_gender || !formData.student_email || !formData.student_phone || !formData.student_dob) {
  ...
  {loading &&
   ...
   <div className="d-flex justify-content-between ">
     <span className="d-flex justify-content-start align-items-center ps-4">
       ...
       <Form.Select className="me-3"style={{width:"150px"}} onChange={(e) => {setSelectedCourse(e.target.value)
         setSelectedBatch('')}} value={selectedCourse}>
         {courses.map((course) => (
